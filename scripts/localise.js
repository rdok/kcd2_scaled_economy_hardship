...
async function batchTranslate(texts, targetLang) {
  try {
    ...
    throw new Error(
      `Batch translation failed for language: ${targetLang}. Error: ${error.message}`,
    );
  }
}
...
async function processXML(inputFilePath) {
  try {
    ...
    const englishTexts = rows.map((row) =>
      row.Cell[1] && row.Cell[1].trim() !== "" ? row.Cell[1] : null,
    );
    ...
    for (const language in languageCodes) {
      ...
      if (textsToTranslate.length === 0) {
        console.log(
          `No valid English texts to translate for ${language}. Skipping...`,
        );
        continue;
      }
      ...
      try {
        ...
        const translatedTexts =
          language === "English"
            ? textsToTranslate
            : await batchTranslate(textsToTranslate, languageCodes[language]);
        ...
        for (let i = 0; i < rows.length; i++) {
          if (englishTexts[i] !== null) {
            translatedData.Table.Row.push({
              Cell: [rows[i].Cell[0], translatedTexts[translationIndex]],
            });
            console.log(
              `Processed row ${i + 1} for ${language}: "${translatedTexts[translationIndex]}"`,
            );
            translationIndex++;
          } else {
            translatedData.Table.Row.push({
              Cell: [rows[i].Cell[0], rows[i].Cell[1]],
            });
          }
        }
